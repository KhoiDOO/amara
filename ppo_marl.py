...
import json
from record import RecordEpisodeStatistics
...
def parse_args():
    ...
    parser.add_argument("--total-timesteps", type=int, default=20000000,  # CleanRL default: 2000000 #Test: 12000
        help="total timesteps of the experiments")
    parser.add_argument("--learning-rate", type=float, default=2.5e-4,
        help="the learning rate of the optimizer")
    parser.add_argument("--num-envs", type=int, default=256, #12000
...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            if "episode" in info:
                acum_rew = info['episode']['r'].tolist()
                epic_len = info['episode']['l'].tolist()

                for idx, (_rew, _len) in enumerate(zip(acum_rew, epic_len)):
                    player_idx = idx % 2

                    if _rew != 0:
                        print(
                            f"global_step={global_step}, {player_idx}-ep_re={_rew}--ep_len={_len}"
                        )
